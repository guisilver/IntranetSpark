...
public class ConciliacaoLFMB implements Serializable {
	...
	private List<intra_conciliacao_lf> listaConc;
	...
	public void init() {
		...
		this.dataFim = new DateTime().plusDays(1).withHourOfDay(1).withMinuteOfHour(59).withSecondOfMinute(59).toDate();
	...
	public List<intra_conciliacao_lf> getListaConc() {
		return listaConc;
	}

	public void setListaConc(List<intra_conciliacao_lf> listaConc) {
	...
	public boolean possuiImg(String tipo_lcto, int nrolancto, ConciliacaoLFDAO dao) {
	...
	public void pesquisarConciliados() {
		try {
			this.valorTotal = 0;
			ConciliacaoLFDAO dao = new ConciliacaoLFDAO();
			...
			for (intra_conciliacao_lf aux : this.listaConc) {
	...
	public void geraRelConciliacao() {
		try {
			...
			this.relatorio = rju.geraRelConciliacaoLF(parametros, "Conciliacao", "Conciliacao", 1, this.listaConc);
